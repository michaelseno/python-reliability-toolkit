...
Current HITL scan-pack/report correction implements runtime execution/result capture for `core_reliability_scan`, records one sanitized scan-pack row per standard scenario per enabled endpoint, includes bounded `burst_stability` as the only approved standard resilience-style check, adds sanitized scan-results CSV export, and redesigns the static HTML report around executive verdict, KPIs, findings, endpoint scorecards, scan-pack matrices, test-level evidence/remediation, latency/availability, methodology/scope, privacy, and export sections.
...
- `reliabilitykit/core/scenario_registry.py` — public-safe remediation and execution-type metadata for scan-pack scenarios, with bounded-language `burst_stability` copy.
- `reliabilitykit/reporting/audit.py` — sanitized endpoint-cycle CSV, scan-results CSV, report view-model rollups, and redesigned static HTML audit report generation.
...
- `examples/api_reliability_audit/README.md` — additionally documents scan-pack report content, bounded `burst_stability`, and scan-results CSV output.
...
`rk audit run --config ...` now resolves `core_reliability_scan` at runtime and persists sanitized `scan_results` in the audit result JSON. `rk audit generate-report --id ...` now emits both `audit_sanitized.csv` and `audit_scan_results_sanitized.csv`, then links those artifacts from the redesigned static HTML report.
...
Scan-pack metadata is persisted in sanitized form through `ScanPackExecutionPlan`, `ScenarioRuntimeDefinition`, and `EndpointScanResult`. The scan-results CSV uses the approved columns: `audit_id`, `check_cycle_id`, `endpoint_id`, `method`, `path`, `scan_pack_id`, `scenario_id`, `scenario_name`, `category`, `severity_if_failed`, `status`, `rationale`, `evidence_summary`, `remediation`, `observed_at`, `affected_cycle_ids`, `sample_count`, `not_run_reason`, `not_applicable_reason`, `raw_data_included`.
...
- Sanitized scan-results CSV: `.reliabilitykit/audits/reports/<audit_id>/audit_scan_results_sanitized.csv`
...
- HITL scan-pack correction: standard audit resolves `core_reliability_scan` from `scan_packs.py`; every enabled endpoint receives one result row for each standard scenario. `baseline_health`, `repeated_stability`, `burst_stability`, `timeout_sensitivity`, and `response_consistency` run bounded sanitized request observations. Validation payload scenarios are recorded as `not_applicable` when no approved endpoint-specific payload schema/contract exists, avoiding synthetic/destructive payloads by default. Auth negative scenarios are recorded as `not_applicable` when no bearer auth config exists.
- Bounded `burst_stability`: implemented as a fixed small per-endpoint scenario with maximum concurrency `3`, maximum total requests `5`, maximum wall-clock duration `10s`, no ramp-up, no sustained/soak duration, no throughput/capacity objective, no cross-endpoint simultaneous burst by default because endpoints are iterated sequentially, and no extra retries.
- Report redesign: HTML now renders an executive verdict/reliability score, KPI cards, prioritized findings/actions, endpoint health scorecards, per-endpoint scan-pack matrices, test-level detail cards, latency/availability summaries, export links, methodology/scope, privacy, delivery, retention, responsive layout, embedded CSS only, wrapping-safe long values, and no external assets/scripts.
...
Standard bounded `burst_stability` is the only resilience-style check allowed in the default scan pack and does not require the optional add-on approval gate. All other load, stress, soak, capacity, chaos, destructive, fault-injection, or broader resilience tests remain excluded. Generated report/CSV evidence is sanitized metadata only and intentionally omits raw responses, raw headers, raw bodies, raw logs, trace logs, stack traces, bearer tokens, and secret references.
...
Scenario results explicitly represent unsupported/missing execution context as `not_applicable` or `incomplete` with rationale rather than silently omitting rows. Burst stability returns `incomplete` if all approved bounded observations cannot be captured within the configured limit.
...
Report rollups are computed from sanitized result models only. CLI output now includes the scan-results CSV path during report generation.
...
- Generic negative payload scenarios require an approved endpoint-specific payload contract; without one they are reported as `not_applicable` instead of sending synthetic payloads.
- `timeout_sensitivity` uses a conservative short timeout observation and sanitized status/timing metadata only; it does not perform fault injection.
...
- `./.venv/bin/python -m pytest tests/unit/test_api_reliability_audit_mvp.py` — initially failed after scan-pack/report implementation because the new report rendered unsanitized legacy endpoint error summaries and because the unapproved-scenario test matched the substring `load` in `invalid_payload_handling`; both were corrected.
- `./.venv/bin/python -m pytest tests/unit/test_api_reliability_audit_mvp.py tests/unit/test_cli_commands.py tests/unit/test_packaging_entrypoints.py` — passed after scan-pack/report correction: 34 passed in 0.46s.
- `./.venv/bin/python -m pytest tests/unit` — passed after scan-pack/report correction: 66 passed in 0.50s.
- `./.venv/bin/python -m pytest tests/unit` — passed after auth-negative scenario refinement: 66 passed in 0.49s.
...
- Negative validation scenarios are represented but not actively payload-tested without an approved schema/payload contract, to preserve safety and avoid unintended destructive behavior.
- The report computes deterministic local rollups from the latest result bundle; multi-cycle aggregation beyond persisted result content remains operator-controlled.

## 12. Commit Status
Initial MVP committed as `856b2fd` (`feat(backend): implement api reliability audit mvp`). HITL corrections committed as `af20224` (`fix(backend): apply audit mvp hitl corrections`). Previous usability correction committed before this HITL loop. Streamlined CLI correction committed as `68860be` (`fix(backend): streamline audit cli workflow`). Packaging correction committed as `fa02278` (`fix(backend): repair audit cli editable install`). Scan-pack/report correction commit pending at report-update time.
